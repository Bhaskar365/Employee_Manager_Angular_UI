...
export class MatEditDialogCompComponent implements OnInit{
  ...
  ngOnInit() {
   ...
   editFormSubmit() {
          this.serv.updateEmployees(this.editForm.value.userID,this.editForm.value).pipe(catchError((err)=>{
            this.toast.error({detail:'Error editing employee', summary:'Edit not successful', duration: 5000})
            return err;
          })).subscribe((res:any)=>{
              this.toast.success({ detail:'Employee Edited', summary: 'Edit Successful' ,duration:5000});
